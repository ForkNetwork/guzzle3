...
class ExceptionCollectionTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testAggregatesExceptions()
    {
        ...
        $e->add($exceptions[0]);
        $e->add($exceptions[1]);
        $this->assertRegExp('~\(Exception\) (.+)/tests/Guzzle/Tests/Common/Exception/ExceptionCollectionTest\.php line~', $e->getMessage());
    ...
    public function testCanAddSelf()
    {
        ...
        $this->assertRegExp('~\(Exception\) (.+)/tests/Guzzle/Tests/Common/Exception/ExceptionCollectionTest\.php line~', $message);
        $this->assertContains("\n    Test\n\n    #0", $message);
        $this->assertContains("(Guzzle\\Common\\Exception\\ExceptionCollection)", $message);
        $this->assertContains("\n\n    Meta description!\n\n", $message);
        $this->assertContains("/tests/Guzzle/Tests/Common/Exception/ExceptionCollectionTest.php line ", $message);
        $this->assertContains("\n        Test 2\n\n        #0", $message);
        $this->assertContains("/tests/Guzzle/Tests/Common/Exception/ExceptionCollectionTest.php line ", $message);
